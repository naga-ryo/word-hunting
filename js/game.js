...
async function main() {
    ...
    if (localStorage.getItem("reload") === "none") {
        ...
        await customAlert("不正な操作を検知しました。");
    ...
    if (!result || !result.success) {
        await customAlert("ゲーム開始に失敗しました。もう一度試してください。");
        window.location.href = "index.html";
        return;
    }
...
async function getHint() {
    ...
    if (await customConfirm("本当にヒントを使用しますか？" + caution, "light")) {
        try {
            ...
            if (!result.success) {
                await customAlert("ヒントの取得に失敗しました");
            ...
            await customAlert("ヒントの取得中にエラーが発生しました");
            document.querySelector(".hint").remove();
            document.getElementById("buttons").style.display = "flex";
        }
    }
...
async function gameQuit(){
    const result = await customConfirm("本当にゲームを中断しますか？");
    if(result){
        startLoading();
        try {
...
async function answerCheck(){
    ...
    try {
        ...
        await customAlert("解答の送信に失敗しました。");
        window.location.href = "index.html";
    }
}
